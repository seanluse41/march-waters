  ...
  <form method="POST" use:enhance={handleEnhance}>
    ...
    {#if form?.success}
      <div class="p-4 mb-6 text-sm text-green-800 rounded-lg bg-green-50">
        <span class="font-medium">{$_("contact.success.title")}</span> {$_("contact.success.message")}
      </div>
    {/if}
    ...
    <div class="mb-6">
      ...
      <Input id="email" name="email" type="email" placeholder={$_("contact.form.emailPlaceholder")} class="pl-10">
        {#snippet left()}
    ...
    <div class="mb-6">
      ...
      <Input id="phone" name="phone" type="tel" placeholder={$_("contact.form.phonePlaceholder")} class="pl-10">
        {#snippet left()}
    ...
    <!-- Reason for Contact -->
    <div class="mb-6">
      <p class="mb-4 font-semibold text-slate-700">{$_("contact.form.reasonHeading")}</p>
      <div class="flex flex-wrap gap-4">
        <Checkbox inline name="childcare" class="me-2">{$_("nav.childcare")}</Checkbox>
        <Checkbox inline name="bodyChoice" class="me-2">{$_("nav.speak")}</Checkbox>
        <Checkbox inline name="midwife" class="me-2">{$_("nav.consult")}</Checkbox>
        <Checkbox inline name="event" class="me-2">{$_("contact.form.event")}</Checkbox>
        <Checkbox inline name="other" bind:checked={reasons.other} class="me-2">{$_("contact.form.other")}</Checkbox>
      </div>
      {#if reasons.other}
        <div class="mt-2">
          <Input 
    ...
    <!-- Message Field -->
    <div class="mb-6">
      <Label for="message" class="mb-2 block text-slate-700">{$_("contact.form.message")}</Label>
      <Textarea 
        id="message" 
        name="message"
